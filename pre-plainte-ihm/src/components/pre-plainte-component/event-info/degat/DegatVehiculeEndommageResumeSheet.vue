<template>
  <div>
    <v-sheet class="pa-4 mb-4 objet-degrade-resume">
      <div class="d-flex flex-wrap align-center justify-space-between gap-2 mb-3">
        <span class="text-subtitle-1 font-weight-medium">
          {{ t("dommages.vehiculeEndommageNumero", { n: index + 1 }) }}
        </span>
        <div class="d-flex gap-2">
          ...
          <v-btn variant="text" color="error" size="small" @click="emit('supprimer', index)">
            {{ t("dommages.supprimerVehiculeEndommage") }}
          </v-btn>
        </div>
      </div>
      <v-row dense>
        <v-col v-if="obj.categorieObjet" cols="6">
        ...
        <v-col v-if="obj.fabricant?.code !== 'AUTRE' && (obj.modele || obj.modeleAutre)" cols="6">
        ...
        <v-col v-if="obj.valeurReelle" cols="6">
        ...
        <v-col v-if="obj.sousCategorie === 'velos' && obj.numeroCadre" cols="6">
        ...
        <v-col v-if="obj.dateAchat" cols="6">
    ...
    >
      {{ t("dommages.ajouterAutreVehiculeEndommage") }}
    </v-btn>
  </div>
</template>
