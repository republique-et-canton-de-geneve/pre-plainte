...
export function useMobileKeyboardInset() {
  onMounted(() => {
    ...
    const updateInset = () => {
      const inset = Math.max(0, globalThis.innerHeight - visualViewport.height - visualViewport.offsetTop);
    ...
    globalThis.addEventListener("orientationchange", updateInset);
    updateInset();

    onUnmounted(() => {
      visualViewport.removeEventListener("resize", updateInset);
      visualViewport.removeEventListener("scroll", updateInset);
      globalThis.removeEventListener("orientationchange", updateInset);
